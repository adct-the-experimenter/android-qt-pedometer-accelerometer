...
//class inherits publicly from QAccelerometer and publicly for QQmlPropertyValueSource
class SpecialAccelerometerPedometer : public QAccelerometer,  public QQmlPropertyValueSource
{
    Q_OBJECT
    ...
    Q_PROPERTY(qreal roll READ roll WRITE setRollValue NOTIFY rollValueChanged) //roll property
    Q_PROPERTY(qreal pitch READ pitch WRITE setPitchValue NOTIFY pitchValueChanged) // pitch property
    Q_PROPERTY(qreal yaw READ yaw WRITE setYawValue NOTIFY yawValueChanged) //yaw property
    Q_PROPERTY(qreal stepCount READ stepCount WRITE setNumberOfSteps NOTIFY stepCountChanged) //step count property
    Q_PROPERTY(qreal tangent_line_y_intercept READ tangentLineSlope WRITE setTangentLineSlope NOTIFY tangentLineSlopeChanged) //tangent line slope property
    Q_PROPERTY(qreal tangent_line_slope READ tangentLineIntercept WRITE setTangentLineIntercept NOTIFY tangentLineInterceptChanged)//tangent line intercept property
...
//roll, forward direction
    ...
    //function to invoke from QML side, calls C++ function setRollBasedOnRollCalculatedFromReadings()
    Q_INVOKABLE void calcRollFromReadings(){SpecialAccelerometerPedometer::setRollBasedOnRollCalculatedFromReadings();}
...
//pitch, side direction
    ...
    //function to invoke from QML side, calls C++ function setPitchBasedOnPitchCalculatedFromReadings()
    Q_INVOKABLE void calcPitchFromReadings(){SpecialAccelerometerPedometer::setPitchBasedOnPitchCalculatedFromReadings();}
...
// yaw, vertical direction
    ...
    //function to invoke from QML side, calls C++ function setRollBasedOnRollCalculatedFromReadings()
    Q_INVOKABLE void calcYawFromReadings(){SpecialAccelerometerPedometer::setYawBasedOnYawCalculatedFromReadings();}
...
//Step Counting Algorithm processes
    ...
    Q_INVOKABLE void resetStepCount(){SpecialAccelerometerPedometer::resetNumberOfSteps();}

    //function to invoke from QML side, calls C++ function runStepCountAlgorithm()
    Q_INVOKABLE void runQMLStepCountAlgorithm(){SpecialAccelerometerPedometer::runStepCountAlgorithm();}
...
//QML Property Definition for SpecialAccelerometerPedometer
     ...
     * SpecialAccelerometerPedometer::setTarget() to provide the type with the property
     * to which the value source has been applied. */
...
*/
    ...
    qint8 m_num_steps;
    //vector to hold acceleration values in forward axis direction relative to person and not phone
    ...
    qreal tangent_line_slope_th_freq; //the slope from tangent line from graphing threshold difference vs frequency

    qreal tangent_line_y_intercept_th_freq; //the y-intercept from tangent line from graphing threshold diff vs frequency

    qint16 current_sample_number; //the sample number of data the algorithm is currently on
    ...
    qint16 max_accel_sample_number; //the sample number when max acceleration was recorded
    ...
    bool isThisSampleValid(); //bool for use with step counting algorithm. Tells if next sample is valid for use in algorithm.
