...
void SpecialAccelerometerPedometer::setRollValue(qreal value){m_rollValue = value;}
...
void SpecialAccelerometerPedometer::setRollBasedOnRollCalculatedFromReadings()
{
    ...
    {
        SpecialAccelerometerPedometer::setRollValue(thisRollValue); //set m_roll_value to thisRollValue
...
void SpecialAccelerometerPedometer::setPitchValue(qreal value){m_pitchValue = value;}
...
void SpecialAccelerometerPedometer::setPitchBasedOnPitchCalculatedFromReadings()
{
...
void SpecialAccelerometerPedometer::runStepCountAlgorithm()
{
    ...
    if(SpecialAccelerometerPedometer::isThisSampleValid())
    {
        ...
        //qDebug() << "In start of Step Count Algorithm. Sample is valid! At " << cSampleNum << "\n";


        //get max accel sample number
        qint16 maxSampleNum = SpecialAccelerometerPedometer::getMaxAccelSampleNumber();

        //the calculated difference between positive peak and negative peak acceleration of a single step session
        //a step is counted if difference between max acceleration value and current acceleration is equal to
        //or more than this
        ...
        //(1/(i-k)) = frequency of steps, i = current sample number, k = max accel sample number
        //a = constant from slope of tangent line of th vs (1/(i-k))
        //B = constant from intercept of tangent line of th vs (1/(i-k))
        threshold_difference = (tangent_line_slope_th_freq / (cSampleNum - maxSampleNum))
        ...
        //if difference between maximum acceleration and current acceleration is more than
        ...
        //else if difference between maxmimum acceleration and current acceleration is less than
        //threshold difference
        else
        {
            //if current acceleration is more than max acceleration,
            //assign current acceleration to max acceleration
            if(currentAccel > maxAccel){SpecialAccelerometerPedometer::setMaxAcceleration(currentAccel);}
        }
...
//bool for use with step counting algorithm. Tells if next sample is valid for use in algorithm.
bool SpecialAccelerometerPedometer::isThisSampleValid()
{
    ...
    if(cSample > 0 && cSample < m_acceleration_vector_size)
    {
        ...
        if(thisForwardAccel !=
           m_forward_accelerations.at(cSample - 1))
        {
            //add later, if acceleration is "different" enough from previous acceleration
    ...
    {
        SpecialAccelerometerPedometer::resetCurrentSampleNumber(); //reset current sample number
        SpecialAccelerometerPedometer::resetMaxAccelSampleNumber(); //reset max accel sample number
        m_forward_accelerations[0] = thisForwardAccel; //assign thisForwardAccel to index zero of vector
    }
...
//set max_accel
void SpecialAccelerometerPedometer::setMaxAcceleration(qreal num){max_accel = num;}
// return max_accel
qreal SpecialAccelerometerPedometer::getMaxAcceleration(){return max_accel;}

//increment current_sample_number by 1
void SpecialAccelerometerPedometer::incrementCurrentSampleNumber(){current_sample_number += 1;}
//initialize current_sample_number to one
void SpecialAccelerometerPedometer::resetCurrentSampleNumber(){current_sample_number = 1;}
//return current_sample_number
qint16 SpecialAccelerometerPedometer::getCurrentSampleNumber(){return current_sample_number;}
//set max_accel_sample_number
void SpecialAccelerometerPedometer::setMaxAccelSampleNumber(qint16 thisNum){max_accel_sample_number = thisNum;}
//return max_accel_sample_number
qint16 SpecialAccelerometerPedometer::getMaxAccelSampleNumber(){return max_accel_sample_number;}
//reset max_accel_sample_number
void SpecialAccelerometerPedometer::resetMaxAccelSampleNumber(){max_accel_sample_number = 0;}

void SpecialAccelerometerPedometer::setTangentLineSlope(qreal num){tangent_line_slope_th_freq = num;}
qreal SpecialAccelerometerPedometer::tangentLineSlope() const{return tangent_line_slope_th_freq;}

void SpecialAccelerometerPedometer::setTangentLineIntercept(qreal num){tangent_line_y_intercept_th_freq = num;}
qreal SpecialAccelerometerPedometer::tangentLineIntercept() const{return tangent_line_y_intercept_th_freq;}

/*
 **************************
 **QML Property Defintion**
 **************************
*/

//function to set target
void SpecialAccelerometerPedometer::setTarget(const QQmlProperty &prop){ m_targetProperty = prop; }
